  ...
  try {
    ...
    const p = new Table({
      ...
      rows: resp['projects'].map((p: any) => {
        ...
        let v: any = {} for (const f of fields) {
          v[f] = p[f]
        }
...
export async function listAMSSites(project: string): Promise<void> {
  ...
  try {
    ...
    if (response.status != 200) {
      ...
      throw new Error(`Failed to fetch export data, code: ${
    ...
    Object.keys(sites).forEach((key: string) => {console.log(key)})
  } catch (e) {
    if (e instanceof axios.AxiosError) {
      if (e.response?.status != 200) {
        if (e.response?.status == 401) {
...
export async function listAMSSources(
  ...
  try {
    ...
    let output: any = {} for (const system of s) {
...
export async function listEvents(start: string, end: string): Promise<void> {
  ...
  try {
    ...
    for (const event of events) {
      for (let i = 0; i < properties.length; i++) {
        const p = properties[i] if (event[p]) {
