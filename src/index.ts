...
const datums = new Command("datums").description(
  "Fetch datums from SolarNetwork",
);
...
listlocationsources
  .option("-s, --source <sourceId>", `Source ID to display.`)
...
stream
  .requiredOption(
    "-o, --output <file>",
    "File which exported data is written to",
    undefined,
  )
  .requiredOption(
    "--start <startDate>",
  ...
  .requiredOption("-f, --format <format>", `Header of exported CSV data`)
  .option(
  ...
  .action(async () => {
    ...
    const result = await fetchSNDatums(
      fd,
      src,
      opts["format"],
      opts["start"],
