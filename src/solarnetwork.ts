...
import {AggregatedDatum, AggregatedLocationDatum, ExportDataConfiguration, ExportDatumFilter, ExportDestinationConfiguration, ExportOutputConfiguration, ExportSettingsSpecifier, ExportTask, ExportTypeInfo, getDatums, getLocationDatums, getLocationMeta, getMeasurementDescriptor, getNodeIds, listExportTasks, listExportType, listSources, MeasurementType, parseAggregatedDatums, parseAggregatedLocationDatums, parseRawDatums, parseRawLocationDatums, RawDatum, RawLocationDatum, staleAggregation, StreamMeta, submitExportTask, TaggedDatum, TaggedLocationDatum, TaggedRawDatum, TaggedRawResponse, TaggedStreamResponse} from './solarnetwork_api.js'
import {getDateRanges} from './util.js';


function columnName(c: string): string{const meta = c.indexOf('$')
        ...
        switch (sample.type) {
          ...
          case 'aggregated':
            switch (metaName) {
              ...
              case 'maximum':
                return sample.max.toString() default:
...
function columnValue(c: string, row: TaggedDatum, m: StreamMeta): string {
  ...
  switch (desc.type) {
    case MeasurementType.Instantaneous: {
      switch (row.state) {
        ...
        case 'raw': {
          ...
          const v = i[desc.index] if (v == undefined)
          return ''
    ...
    case MeasurementType.Accumulating: {
      switch (row.state) {
        ...
        case 'raw': {
          ...
          const v = a[desc.index] if (v == undefined)
          return ''
        ...
        case 'aggregated': {
          ...
          switch (metaName) {
            ...
            case 'ending':
              return ending.toString() default:
                  throw new Error(`Unrecognized meta value: ${metaName}`)
          }
        }
      }
    ...
    case MeasurementType.Status: {
      ...
      const v = s[desc.index] if (v == undefined)
      return ''
...
function chunkArray<T>(arr: T[], n: number): T[][] {
    ...
    async function listLocationSourceMeasurements(
      ...
      const datum = result.value.response.data.results[0] let rows =
    ...
    async function listLocationSources(id: string):
      ...
      let rows = [] let collected = new Set()
      let cols =
          [
            {
              name: 'location',
    ...
    export async function listSourceMeasurements(path: string):
      ...
      let rows = [] for (const source of result.value.response.meta) {
        if (source['i']) {
    ...
    interface SNChunk {
      response: TaggedStreamResponse, total: number
    }
    ...
    async function fetchSNDatumsConsumer(
      ...
      for await (const next of chan) {
        ...
        let rows: RawDatum[]|AggregatedDatum[] = [] switch (chunk.state) {
          case 'raw': {
        ...
        for (const row of rows) {
          ...
          if (foundColumns.length != columns.length) {
            const empty = opts['empty'] const partial =
                opts['partial'] const isEmpty =
          ...
          if (opts['sn_recalculate_skip_field']) {
            for (let i = 0; i < columns.length; i++) {
              const c = columns[i] if (c == opts['sn_recalculate_skip_field']) {
                switch (chunk.state) {
                  case 'raw': {
          ...
          for (let i = 0; i < columns.length; i++) {
            ...
            lastNodeID = m['objectId']
              ...
              if (!recalculating &&
                ...
                recalculating = false await staleAggregation(
    ...
    export async function getDefaultFormat(source: string):
        Promise<Result<string, Error>> {
          ...
          const d = datums.value.response.meta[0] let format = 'timestamp,'
          if (d.i) {
            for (const i of d.i) {
              format += (i.toString() + ',')
            }
    ...
    export async function fetchExportTasks():
        Promise<Result<void, Error>> {
          ...
          for (const task of tasks.value) {
            const config = task['config'] const t = task['task']
    ...
    const value = (destinationType as ExportTypeInfo)
        ...
        let outputServiceProperties:
            Record<string, any> = {} for (const prop of outputProps) {
          let [[k, v], t] = prop
        ...
        let destinationServiceProperties:
            Record<string, any> = {} for (const prop of destinationProps) {
          let [[k, v], t] = prop
