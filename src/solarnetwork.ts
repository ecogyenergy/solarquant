...
export async function listSourceMeasurements(
  path: string,
): Promise<Result<void, Error>> {
  const cfg = readConfigFile();
  ...
  for (const source of sources) {
    ...
    try {
      ...
      for (const range of ranges) {
        ...
        if (response.isErr) {
          console.error(
            `Warning: some datums failed to be fetched: ${response.error.message}`,
          );
          continue;
        }
  ...
  for await (const next of chan) {
    ...
    for (const row of rows) {
      ...
      if (foundColumns.length != columns.length) {
        ...
        const isEmpty = haveTimestamp
          ? foundColumns.length == 1
          : foundColumns.length == 0;
        const isPartial = haveTimestamp
          ? foundColumns.length > 1
          : foundColumns.length > 0;
...
export async function fetchSNDatums(
  ...
  if (sources.isErr) {
    return Result.err(
      new Error(`Failed to fetch list of sources: ${sources.error.message}`),
    );
  }
  ...
  try {
    ...
    const p1 = fetchSNDatumsConsumer(
      ...
      chan,
      bar,
      sources.value.length * coefficient,
      ids.value,
      format,
      start,
      end,
    ...
    const sncfg = cfg.sn;
    const p2 = Array.from(Array(parallel).keys()).map(async (i) =>
      fetchSNDatumsProducer(
        sncfg,
        chan,
        bar,
        ids.value,
...
async function fetchExportList(t: string): Promise<Result<void, Error>> {
  ...
  if (result.isErr) {
    return Result.err(
      new Error(`Failed to fetch export types: ${result.error.message}`),
    );
  }
...
export async function startExportTask(opts: any): Promise<Result<void, Error>> {
  ...
  if (compressionTypes.isErr) {
    return Result.err(
      new Error(
        `Failed to fetch compression types: ${compressionTypes.error.message}`,
      ),
    );
  }
  ...
  if (outputTypes.isErr) {
    return Result.err(
      new Error(`Failed to fetch output types: ${outputTypes.error.message}`),
    );
  }
  ...
  if (destinationTypes.isErr) {
    return Result.err(
      new Error(
        `Failed to fetch destination types: ${destinationTypes.error.message}`,
      ),
    );
  }

  // Get the right type
  let compressionType: ExportTypeInfo | undefined = compressionTypes.value.find(
    (t: ExportTypeInfo) => {
      return (
        t.id === opts["compression"] || t.localizedName === opts["compression"]
      );
    },
  );
  ...
  let destinationType: ExportTypeInfo | undefined = destinationTypes.value.find(
    (t: ExportTypeInfo) => {
      return (
        t.id === opts["destination"] || t.localizedName === opts["destination"]
      );
    },
  );
  ...
  const outputProps = opts["outputProp"].map((p: string) => {
    ...
    const value = (outputType as ExportTypeInfo).settingSpecifiers.find(
  ...
  const destinationProps = opts["destinationProp"].map((p: string) => {
    const both = splitOnce(p, ":");
    const value = (destinationType as ExportTypeInfo).settingSpecifiers.find(
      (s: ExportSettingsSpecifier) => s.key === both[0],
    );
    return [both, value];
  });
