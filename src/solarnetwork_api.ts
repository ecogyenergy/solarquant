...
export interface LocationData {
  totalResults: number, startingOffset: number, returnedResultCount: number,
      results: any[]
}
...
export interface TaggedRawLocationResponse {
  state: 'raw', response: LocationResponse
}

export interface TaggedAggregatedLocationResponse {
  state: 'aggregated', response: LocationResponse
}

export interface TaggedRawResponse {
  state: 'raw', response: StreamResponse
}

export interface TaggedAggregatedResponse {
  state: 'aggregated', response: StreamResponse
}
...
function ensureNumberType(value: any):
    number{return (typeof value === 'number') ? value : NaN}
    ...
    AggregatedLocationDatum[] {
      return response.data.results.map(datum => {
        ...
        for (const key in datum) {
          ...
          const base = key
          const average = datum[key] const min =
              datum[base + '_min'] const max = datum[base + '_max']
...
export function parseRawDatums(response: StreamResponse):
    RawDatum[] {
      return response.data.map(datum => {
        const meta: number = datum[0] const timestamp: number =
            datum[1] const m = response.meta[meta]

                               const i_len = m.i ? m.i.length : 0
        const a_len = m.a ? m.a.length : 0
        const s_len = m.s ? m.s.length : 0

        let i: number[] = [] let a: number[] = [] let s: string[] =
            [] let tags: string[] = []
...
export function parseAggregatedDatums(response: StreamResponse):
    AggregatedDatum[] {
      return response.data.map(datum => {
        const meta: number =
            datum[0] const timestamp: [start: number, end: number] =
                datum[1] const m = response.meta[meta]
        ...
        let i: [number, number, number, number][] =
            [] let a: [number, number, number][] = [] let s: string[] =
                [] let tags: string[] = []
...
interface LocationMeta {
  locationId: number
  sourceId: string
  m: Record<string, string>, t: string[]
}
...
export interface ExportTask {
  name: string, dataConfiguration: ExportDataConfiguration,
...
export async function submitExportTask(
  ...
  let headers: Record<string, any> = {} headers[HttpHeaders.DIGEST] =
...
export async function staleAggregation(
  ...
  let headers: Record<string, any> = {} headers[HttpHeaders.X_SN_DATE] =
