...
export async function getNodeIds(
  ...
  try {
    ...
    if (!response.data.success) {
      return Result.err(
        new Error("SolarNetwork API call failed: /solarquery/api/v1/sec/nodes"),
      );
    }
...
export async function listExportType(
  t: string,
  cfg: SNConfig,
): Promise<Result<ExportTypeInfo[], Error>> {
...
export type RawMeasurement =
  | RawInstantaneousMeasurement
  | RawAccumulatingMeasurement
  | RawStatusMeasurement;
...
export type TaggedLocationResponse =
  | TaggedRawLocationResponse
  | TaggedAggregatedLocationResponse;
...
export type TaggedLocationDatum =
  | TaggedRawLocationDatum
  | TaggedAggregatedLocationDatum;
...
export function parseRawLocationDatums(
  response: LocationResponse,
): RawLocationDatum[] {
...
export function parseAggregatedLocationDatums(
  response: LocationResponse,
): AggregatedLocationDatum[] {
  return response.data.results.map((datum) => {
    const samples: Map<string, TaggedAggregatedLocSample | TaggedRawLocSample> =
      new Map();
    const ignore = new Set([
      "created",
      "locationId",
...
export function parseAggregatedDatums(
  response: StreamResponse,
): AggregatedDatum[] {
  return response.data.map((datum) => {
    const meta: number = datum[0];
...
export function getMeasurementDescriptor(
  meta: StreamMeta,
  measurement: string,
...
export async function getDatums(
  ...
  try {
    ...
    if (aggregation) {
      const rawResponse = (
        await axios.get<StreamResponse>(urlString, {
          headers: {
            Authorization: authHeader,
            "X-SN-Date": auth.requestDateHeaderValue,
            "Accept-Encoding": "UTF8",
...
export async function getLocationDatums(
  ...
  try {
    ...
    if (aggregation) {
      const rawResponse = (
        await axios.get<LocationResponse>(urlString, {
          headers: {
            Authorization: authHeader,
            "X-SN-Date": auth.requestDateHeaderValue,
            "Accept-Encoding": "UTF8",
...
export async function getLocationMeta(
  cfg: SNConfig,
  locationId: string,
): Promise<Result<LocationMeta[], Error>> {
  const auth = new AuthorizationV2Builder(cfg.token).saveSigningKey(cfg.secret);
...
export async function submitExportTask(
  ...
  try {
    const response = await axios.post<ExportResponse<null>>(url, js, {
      headers: headers,
    });
